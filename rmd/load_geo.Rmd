...
factor_covariates <- list(
  "GSE116324" = c('treatment_response', 'iss_stage'),
...
for (acc in accessions) {
  ...
  # data size
  dataset_num_rows <- c(dataset_num_rows, nrow(expr))
  dataset_num_cols <- c(dataset_num_cols, sum(sample_ind))
  ...
  # prepend dataset accession to column names
	ind <- colnames(gene_weights) %in% c('variance', 'mad', 'kurtosis',
																			 'dip_stat', 'dip_pval', 'silv_pval')
