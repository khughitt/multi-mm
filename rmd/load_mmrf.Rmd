...
```{r load_mmrf_patient_clinical_metadata, eval = !file.exists(gene_weights_path, pathway_weights_path)}
...
for (covariate in c('iss_stage', 'ecog', 'mm_status')) {
  ...
  if (is.character(response) || length(unique(response)) < 10) {
    ...
    gene_weight_vector <- parApply(cl, as.matrix(mmrf_cpm_counts[, -1]), 1, function(x) {
...
```{r load_mmrf_patient_treatment_metadata, eval = !file.exists(gene_weights_path, pathway_weights_path)}
...
for (treatment in treatments) {
  ...
  if (is.character(response) || length(unique(response)) < 10) {
    ...
    pathway_weight_vector <- parApply(cl, pathway_expr[, -1], 1, function(x) {
      mod <- glm(response ~ x, family = "binomial")
      as.numeric(1 - logLik(mod) / logLik(null))
    })
  }
...
```{r load_mmrf_patient_survival_metadata, eval = !file.exists(gene_weights_path, pathway_weights_path)}
...
gene_fits <- parApply(cl, as.matrix(mmrf_cpm_counts[, -1]), 1, function(x) {
...
gene_dip <- apply(as.matrix(mmrf_cpm_counts[, -1]), 1, dip.test)
...
gene_weights$mmrf_dip_stat <- unlist(lapply(pathway_dip, function(x) { x$statistic }))
gene_weights$mmrf_dip_pval <- unlist(lapply(pathway_dip, function(x) { x$p.val }))
...
```{r mmrf_functional_enrichment, warnings = FALSE, eval = !file.exists(fgsea_path)}
