...
gene_expr_raw <- read_tsv(gene_expr_path)
...
if (!file.exists(pathway_expr_path)) {
  ...
  res <- foreach(gset_num = 1:length(gene_sets), .packages = c('tidyverse')) %dopar% {
...
```{r load_mmrf_patient_clinical_metadata, eval = !stats_avail}
# load patient clinical metadata
mmrf_clin <- read_csv(mmrf_clin_metadata) %>%
...
```{r load_mmrf_patient_treatment_metadata, eval = !stats_avail}
# load patient treatment metadata
mmrf_treatment <- read_csv(mmrf_trmt_metadata)
...
```{r load_mmrf_patient_survival_metadata, eval = !stats_avail}
# load patient survival metadata
# regular and censored overall & progression-free survival
mmrf_surv <- read_csv(mmrf_surv_metadata)
...
gene_stats$MMRF13a_dip_pval <- unlist(lapply(gene_dip, function(x) { x$p.val }))

pathway_dip <- apply(as.matrix(pathway_expr[, -1]), 1, dip.test)
pathway_stats$MMRF13a_dip_pval <- unlist(lapply(pathway_dip, function(x) { x$p.val }))
...
if (!file.exists(fgsea_rsq_path)) {
  ...
  for (covariate in colnames(gene_rsqs)[-1]) {
    ...
    cov_res <- foreach(gene_set = names(gene_sets), .packages = c('fgsea', 'tidyverse')) %dopar% {
      set.seed(1)
      ...
      gset_res <- fgsea(gene_sets[[gene_set]], stats, nperm = FGSEA_NPERM, nproc = FGSEA_NPROC) %>%
        select(-leadingEdge) %>%
        arrange(padj)
