...
for (acc in accessions) {
  ...
  outfile <- file.path('output', sprintf('gene_weights_%s.csv', acc))
...
outfile <- file.path('output', 'gene_weights_MMRF.csv')
```
...
dataset_sizes <- rbind(dataset_sizes, c('mmrf', ncol(mmrf_raw_counts) - 1))
...
for (gs in names(gene_sets)) {
	res <- fgsea(gene_sets[[gs]], xx, nperm = 10000, nproc = num_threads) %>%
		filter(padj < 0.05) %>%
		select(-leadingEdge) %>%
		arrange(padj)
